...
public class InfoController {
    ...
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    public ResponseEntity<MessageResponse> postChatroom(@RequestBody CreateChatRequest createChatRequest) {
        ...
        return ResponseEntity.ok( new MessageResponse("Successfully created new chatroom."));
    }

    @PutMapping("/inviteUser/{chatroomId}")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    public ResponseEntity<MessageResponse> addPlayer (@PathVariable String chatroomId, @RequestBody InviteUserRequest r) {
        ...
        if(!chatroom.getUsers().contains(user)){
            ...
            ChatMessage joinMessage = new ChatMessage(ChatMessage.MessageType.JOIN, LocalDateTime.now(), user.getUsername());
            messagingTemplate.convertAndSend("/topic/" + chatroom.getId(), joinMessage);
            return ResponseEntity.ok(new MessageResponse("User was successfully added to chatroom!"));
        }
        return ResponseEntity.badRequest().body(new MessageResponse("User is already in chatroom!"));
    }

    @PutMapping("/leaveChat/{chatroomId}")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    public ResponseEntity<MessageResponse> leaveChat (@PathVariable String chatroomId, @RequestBody LeaveChatRequest r) {
        User user=userRepository.findByUsername(r.getUsername()).get();
        Chatroom chatroom = chatRepository.findById(chatroomId).get();
        if(chatroom.getUsers().contains(user)){
            ...
            return ResponseEntity.ok(new MessageResponse("User was successfully removed from chatroom!"));
        }
        return ResponseEntity.badRequest().body(new MessageResponse("User is not in chatroom!"));
    }
}
